...
import PollinationsStudio from './components/tools/PollinationsStudio';
...
const App: React.FC = () => {
  ...
  const [openRouterApiKey, setOpenRouterApiKey] = useState<ApiKey>({ key: '', status: 'unknown' });
  const [openaiApiKey, setOpenaiApiKey] = useState<ApiKey>({ key: '', status: 'unknown' });
  const [pollinationsApiKey, setPollinationsApiKey] = useState<ApiKey>({ key: '', status: 'unknown' });
  ...
  useEffect(() => {
    ...
    setOpenRouterApiKey(settings.openRouterApiKey || { key: '', status: 'unknown' });
    setOpenaiApiKey(settings.openaiApiKey || { key: '', status: 'unknown' });
    setPollinationsApiKey(settings.pollinationsApiKey || { key: '', status: 'unknown' });
  }, []);
  ...
  const renderPage = () => {
      ...
      if (Object.values(ToolType).includes(currentPage as ToolType)) {
          ...
          switch(currentPage) {
            ...
            case ToolType.PollinationsStudio:
                return <PollinationsStudio
                           onNavigateHome={() => setCurrentPage('home')}
                           logStatus={logStatus}
                           setError={setError}
                           setWarning={setWarning}
