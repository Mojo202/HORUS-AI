...
| ملاحظة: تم الإنشاء لأغراض فنية وتعليمية بحتة. يجب أن تكون الملامح مستوحاة من الطبيعة البشرية ولكنها ليست تمثيلاً لأي شخص حقيقي. استخدم الرمزية المجردة للمواضيع الحساسة.
    ...
    if (imageModel === 'gemini-2.5-flash-image') {
        log("⚡️ Using Gemini Flash Image (generateContent)...");
        ...
        if (!imagePart?.inlineData) {
            throw new Error('فشل إنشاء الصورة باستخدام Gemini Flash، لم يتم إرجاع بيانات. قد يكون السبب هو حظر المحتوى.');
        }
        base64ImageBytes = imagePart.inlineData.data;
        mimeType = imagePart.inlineData.mimeType;

    } else {
        log("🎨 Using Imagen (generateImages)...");
    ...
    const imageDataUrl = `data:${mimeType};base64,${base64ImageBytes}`;
