...
const OPENROUTER_API_KEY = 'openRouterApiKey_v1';
const OPENAI_API_KEY = 'openaiApiKey_v1';
const POLLINATIONS_API_KEY = 'pollinationsApiKey_v1';
...
const _loadSettings = (): AppSettings => {
    ...
    return {
      ...
      openRouterApiKey: safeJsonParse<ApiKey>(OPENROUTER_API_KEY, { key: '', status: 'unknown' }),
      openaiApiKey: safeJsonParse<ApiKey>(OPENAI_API_KEY, { key: '', status: 'unknown' }),
      pollinationsApiKey: safeJsonParse<ApiKey>(POLLINATIONS_API_KEY, { key: '', status: 'unknown' }),
    };
};

const _saveSettings = (settings: Partial<AppSettings>): void => {
    ...
    if (settings.openRouterApiKey !== undefined) {
        const keyToSave = { key: settings.openRouterApiKey.key.trim(), status: settings.openRouterApiKey.status || 'unknown' };
        localStorage.setItem(OPENROUTER_API_KEY, JSON.stringify(keyToSave));
    }
    if (settings.openaiApiKey !== undefined) {
        const keyToSave = { key: settings.openaiApiKey.key.trim(), status: settings.openaiApiKey.status || 'unknown' };
        localStorage.setItem(OPENAI_API_KEY, JSON.stringify(keyToSave));
    }
    if (settings.pollinationsApiKey !== undefined) {
        const keyToSave = { key: settings.pollinationsApiKey.key.trim(), status: settings.pollinationsApiKey.status || 'unknown' };
        localStorage.setItem(POLLINATIONS_API_KEY, JSON.stringify(keyToSave));
    }
};

// --- Public API Manager Object ---
...
export const apiKeyManager = {
  ...
  getActiveGeminiApiKey: (): string | null => {
    const keys = _loadSettings().geminiApiKeys;
  ...
  hasGeminiKeys: (): boolean => {
    return _loadSettings().geminiApiKeys.length > 0;
  },

  getTotalGeminiKeys: (): number => {
    return _loadSettings().geminiApiKeys.length;
  },
